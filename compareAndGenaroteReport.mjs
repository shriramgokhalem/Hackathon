...
for (let page in urls) {
    ...
    if (fs.existsSync(`./result/`+`${baselineRunNumber}/`+`${page}`+`.json`)) {
        ...
        fs.writeFileSync(`./result/`+`${runNumber}/`+`${page}`+`_Comparison.html`, template, 'utf8');
        ...
        console.log('no baseline report exists for ', `${page}`);
